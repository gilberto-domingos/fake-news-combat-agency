"""
1) Buscar MonitoringTarget(keywords) is_active no banco
   → monitoring_target_write_interface.py
   → monitoring_target_implement.py
   Receber:
   → MonitoringTarget ativos cadastrados no banco
   Fazer:
   → buscar os alvos que possuem monitoramento ativo
   → retornar as keywords cadastradas do MonitoringTarget
        ↓
3) Enviar essas keywords para o serviço de pesquisa
   → search_content_service.py
   Receber:
   → keywords pertencentes ao MonitoringTarget
   Fazer:
